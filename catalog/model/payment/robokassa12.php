...
class ModelPaymentRobokassa12 extends Model {
  	...
  	public function getMethod($address, $total) 
	{
		...
		if( $STORE_ID )
		{
			...
			$CONFIG['robokassa_minprice'] = $this->config->get('robokassa_minprice_store');
			$CONFIG['robokassa_maxprice'] = $this->config->get('robokassa_maxprice_store');
		...
		else
		{
			...
			$CONFIG['robokassa_minprice'] = $this->config->get('robokassa_minprice');
			$CONFIG['robokassa_maxprice'] = $this->config->get('robokassa_maxprice');
		...
		if( !empty($CONFIG['robokassa_minprice'][$this->INDEX]) )
		$robokassa_minprice = $CONFIG['robokassa_minprice'][$this->INDEX];
	
		if( !empty($CONFIG['robokassa_maxprice'][$this->INDEX]) )
		$robokassa_maxprice = $CONFIG['robokassa_maxprice'][$this->INDEX];
		
		$subtotal = $this->cart->getSubTotal();		
		if( !empty( $this->session->data['shipping_method'] ) && !empty($this->session->data['shipping_method']['cost']) )
		...
		if ($status) 
		{
			...
			if(  $CONFIG['robokassa_icons'] && file_exists(DIR_IMAGE.$robokassa_images[$this->INDEX]) )
			{
				...
				$image = '<table style="width: 100%;"><tr><td style="vertical-align: middle;  text-align: left; width: 30px"><img src="'.$img_url.'"></td><td style="vertical-align: middle; text-align: left; " valign=middle>'.$robokassa_methods[$this->INDEX][$this->config->get('config_language')].'</td></tr></table>';
			}
			else
			{
