...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		$query = $this->db->query("SELECT DISTINCT *, pd.name AS name, p.image, m.name AS manufacturer, 
            ...
            (SELECT p2d.download_id FROM " . DB_PREFIX . "product_to_download p2d 
                WHERE p.product_id = p2d.product_id LIMIT 1) AS download_id,  
		...
		if ($query->num_rows) {
			return array(
				...
				'top_product'	   => $query->row['top_product']
			);
		} else {
			return false;
		}
    ...
    public function robot2($id, $name){
        $this->db->query("UPDATE " . DB_PREFIX . "product_description SET 
        name = '" . $this->db->escape(html_entity_decode($name, ENT_QUOTES, 'UTF-8')) . "' 
    ...
    public function getProductDownloads($download_id) {

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "download d
