	...
	public function index() {
		...
		if ($this->user->isLogged() && $this->user->hasPermission('modify', 'sale/order')) {
					...
					if ($product_info) {
						...
						if (isset($order_product['order_option'])) {
							foreach ($order_product['order_option'] as $option) {
								if ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'image' || $option['type'] == 'table') {
			...
			foreach ($products as $product) {
				...
				if ($this->request->post['order_product']) {
				 	foreach ($this->request->post['order_product'] as $value) {
			...
			foreach ($results as $result) {
				if ($this->config->get($result['code'] . '_status') || $result['code'] === 'discount') {
					$this->load->model('total/' . $result['code']);

					if (isset($this->request->post['discount_price']) && $result['code'] === 'discount') {
						$this->{'model_total_' . $result['code']}->getTotal($json['order_total'], $total, $taxes, true);
					} else {
			...
			if (isset($this->request->post['order_total'])) {
				...
				foreach ($json['order_total'] as $key=>$value) {
					if ($value['code'] === 'sub_total') {
						$json['order_total'][$key]['value'] = number_format($custom_total, 4, '.', '');
						$json['order_total'][$key]['text'] = number_format($custom_total, 4, '.', ',').' р.';
					} elseif($value['code'] === 'discount') {
						//todo
						$current_discount = null;
						foreach ($json['discounts'] as $discount) {
							if ($this->request->post['discount_price'] === $discount['percent']) {
								$current_discount = $discount;
								$discount_price = $custom_total * $current_discount['percent'] / 100;
								$json['order_total'][$key]['value'] = number_format($discount_price, 4, '.', '');
								$json['order_total'][$key]['text'] = '-' .number_format($discount_price, 4, '.', ',').' р.';
								$json['order_total'][$key]['title'] = 'Скидка ('.$discount['name'].')';
								$custom_total -= $json['order_total'][$key]['value'];
								break;
							} elseif($this->request->post['discount_price'] == 0) {
								$delete_key = $key;
							}
						}
				...
				if ($delete_key !== -1) {
					array_splice($json['order_total'], $delete_key, 1);
				}
			}
