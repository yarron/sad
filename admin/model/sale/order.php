...
class ModelSaleOrder extends Model {
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			$discount_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_total` WHERE code='discount' AND order_id = '" . (int)$order_id . "'");

			if ($discount_query->num_rows) {
				$discount_price = $discount_query->row['value'];
				$discount_method = $discount_query->row['title'];
			} else {
				$discount_price = '';
				$discount_method = '';
			}
